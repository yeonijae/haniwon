...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "/bin/bash -l -c 'export PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$HOME/.local/bin:$HOME/.cargo/bin:/opt/homebrew/bin:$PATH; _D="$CLAUDE_PROJECT_DIR"; if [[ "$_D" =~ ^[a-zA-Z]:[\\/] ]]; then CLAUDE_PROJECT_DIR="/mnt/${_D:0:1,,}/${_D:3//\\//}"; export CLAUDE_PROJECT_DIR; fi; uv run \"$CLAUDE_PROJECT_DIR/.claude/hooks/moai/post_tool__code_formatter.py\"'"
          timeout: 30
        - type: command
