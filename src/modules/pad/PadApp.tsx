...
import { TodaySchedule, DoctorDashboard, CompactPatientStatus, QuickChat, ThemeToggle, PrescriptionPendingList, PrescriptionInputModal } from './components';
...
const DoctorView: React.FC<DoctorViewProps> = ({ doctor, onBack }) => {
  ...
  // 처방 입력 모달 상태
  const [prescriptionModalPackage, setPrescriptionModalPackage] = useState<HerbalPackage | null>(null);
  ...
  return (
    <div className={`min-h-screen ${themeStyles.container} flex flex-col`} style={{ fontSize: `${fontSize}px` }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <PrescriptionPendingList
              ...
              onPatientClick={(patientId, chartNumber) => {
                console.log('Patient clicked from prescription:', patientId, chartNumber);
              }}
              onPrescriptionClick={(pkg) => {
                setPrescriptionModalPackage(pkg);
              }}
            />
      ...
      {/* 처방 입력 모달 */}
      {prescriptionModalPackage && (
        <PrescriptionInputModal
          pkg={prescriptionModalPackage}
          onClose={() => setPrescriptionModalPackage(null)}
          onSuccess={() => {
            setPrescriptionModalPackage(null);
            loadData(); // 데이터 새로고침
          }}
        />
      )}
